...
const int defaultWindowScale = 2;
...
const int keyboardControls[2][8] = {
    ...
    {SDL_SCANCODE_UP, SDL_SCANCODE_DOWN, SDL_SCANCODE_LEFT, SDL_SCANCODE_RIGHT,
        SDL_SCANCODE_J, SDL_SCANCODE_K, SDL_SCANCODE_U, SDL_SCANCODE_I},
...
int main(int argc, const char * argv[])
{
	...
	windowFlags |= SDL_WINDOW_MAXIMIZED;
        ...
        window = SDL_CreateWindow(windowTitle, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,  300, 240, windowFlags);// SCREEN_WIDTH * defaultWindowScale, SCREEN_HEIGHT * defaultWindowScale, windowFlags);
        renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
	int curW, curH;
	SDL_GetRendererOutputSize(renderer, &curW, &curH);
	SDL_RenderSetScale(renderer, (double)curW/300,(double)curH/240);
	//SDL_RenderSetLogicalSize(renderer, 320, 240);
        texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ABGR8888, SDL_TEXTUREACCESS_STREAMING, 160, 128);//SCREEN_WIDTH, SCREEN_HEIGHT);
        ...
        updateScreenRect(320, 256);//SCREEN_WIDTH * defaultWindowScale, SCREEN_HEIGHT * defaultWindowScale);
